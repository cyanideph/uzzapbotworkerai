...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const suppliedSystem = messages.filter((message) => message.role === "system").map((message) => message.content.trim()).filter(Boolean).join("\n");
      const aiMessages = [
        { role: "system", content: suppliedSystem ? `${SYSTEM_PROMPT}\n\nAdditional application instructions:\n${suppliedSystem}` : SYSTEM_PROMPT },
      ...
      // Preserve valid Uzzap [cNN] tags. Remove only forbidden/invalid tags.
